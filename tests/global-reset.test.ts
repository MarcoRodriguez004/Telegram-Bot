...
describe("global database reset", () => {
  it("requires three exact confirmations before deleting every application row", async () => {
    ...
    for (const table of ["users", "tasks", "reminders", "expenses", "notes", "saved_folders", "persistent_notifications", "notification_preferences", "user_vehicles", "contingency_preferences", "contingency_state", "edit_sessions", "conversation_context", "pending_conversation", "conversation_drafts", "conversation_confirmations", "conversation_history", "pending_folder_saves", "data_imports", "operational_alerts", "storage_status", "processed_updates", "database_reset_confirmations", "broadcast_confirmations"]) {
