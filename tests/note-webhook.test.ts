...
type StoredNote = { userId: number; content: string; url: string | null };
...
function createNoteDb() {
          ...
          return {
            async run() {
              ...
              if (query.startsWith("INSERT INTO notes")) {
                notes.push({ userId: Number(values[0]), content: String(values[1]), url: values[2] as string | null });
  ...
  return { db: db as unknown as D1Database, notes };
}
...
describe("note webhook flow", () => {
  it("persists a link with context and confirms it", async () => {
    const { db, notes } = createNoteDb();
    ...
    const request = new Request("https://bot.test/telegram/webhook", {
      ...
      body: JSON.stringify({
        ...
        message: {
          ...
          text: "guardar este link para leer luego: https://example.com",
        },
      }),
    });
    ...
    expect(notes).toEqual([{ userId: 1, content: "para leer luego", url: "https://example.com" }]);
    expect(sentMessages[0]).toMatchObject({ chat_id: 42 });
    expect(sentMessages[0].text).toContain("Nota guardada");
  });
});
