...
describe("deleteUserData", () => {
  it("deletes personal records before the user in one batch", async () => {
    ...
    expect(batchedStatements.map(({ query, values }) => ({ query, values }))).toEqual([
      ...
      { query: "DELETE FROM user_vehicles WHERE user_id = ?", values: [7] },
      { query: "DELETE FROM contingency_preferences WHERE user_id = ?", values: [7] },
      ...
      { query: "DELETE FROM conversation_history WHERE user_id = ?", values: [7] },
