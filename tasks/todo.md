...
- [ ] Añadir contratos seguros para mensajes de texto entrantes y respuestas de texto salientes.
- [ ] Añadir deduplicación de `wamid` y vínculo explícito del remitente WhatsApp con el usuario existente.
- [ ] Implementar el cliente de Meta con timeout, errores no sensibles y token únicamente desde Wrangler secret.
- [ ] Conectar WhatsApp al flujo conversacional actual y adaptar respuestas con teclado a texto.
- [ ] Rechazar remitentes no autorizados sin efectos laterales y conservar la idempotencia del webhook.
- [ ] Añadir documentación de configuración, rotación del token expuesto y prueba manual.
- [ ] Ejecutar pruebas, lint, typecheck y build; revisar el diff en busca de secretos antes de desplegar.
