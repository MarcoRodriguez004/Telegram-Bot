...
La consulta de contingencia combina CAMe con el boletín de la Comisión Ambiental publicado en gob.mx: CAMe conserva prioridad porque contiene el detalle operativo de hologramas y terminaciones; gob.mx funciona como corroboración o respaldo. Si gob.mx presenta una validación anti-bot, el bot continúa usando CAMe y lo informa de forma no bloqueante. `/hoy_no_circula` fuerza una consulta actual aunque el mismo boletín ya se haya notificado.

El administrador puede preparar una difusión con `/difundir <mensaje>` o `/difundir_estado`. Antes de enviar, el bot muestra el número de chats conocidos y exige tres confirmaciones exactas. La difusión no depende de las preferencias de alertas CAMe y solo alcanza chats que ya interactuaron con el bot; Telegram no permite descubrir usuarios que nunca lo han contactado.
