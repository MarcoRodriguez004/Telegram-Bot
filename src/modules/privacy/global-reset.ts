...
export async function deleteAllData(db: D1Database): Promise<void> {
  await db.batch([
    ...
    db.prepare("DELETE FROM user_vehicles"),
    db.prepare("DELETE FROM contingency_preferences"),
    db.prepare("DELETE FROM contingency_state"),
    ...
    db.prepare("DELETE FROM conversation_history"),
    ...
    db.prepare("DELETE FROM operational_alerts"),
